...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "elpAudio"
...
Section "elpAudio" Installer
  ...
  ; extract files
  nsExec::Exec '"$INSTDIR\7z.exe" -y x elpAudio.zip' 
  Delete "$INSTDIR\elpAudio.zip"
...
Section "Start Menu Shortcuts" SMShortucts
  ...
  CreateShortcut "$SMPROGRAMS\elpAudio\elpAudio.lnk" "$INSTDIR\elpAudio.exe"
...
Section "Desktop Shortcuts" DShortcuts
  ...
  CreateShortcut "$DESKTOP\elpAudio.lnk" "$INSTDIR\elpAudio.exe"
